...
A Deno / Deno.dlopen consumer of
...
```bash
git clone --recurse-submodules https://github.com/flags-2-env-test/deno-app
cd deno-app
docker build -t f2e-fixture-deno .
docker run --rm f2e-fixture-deno
```
...
docker run --rm -v /tmp/mutant.toml:/app/.cli-flags.toml:ro f2e-fixture-deno
